module.exports = (function() {
  ...
  function callAllRequest(tdxApi, sourceId, destStream, endIndex) {
    ...
    return tdxApi.getDatasetDataAsync(sourceId, genderFilter, null, genderOptions)
      .then((result) => {
        return Promise.all(_.map(result.data, (eachObj) => {
          ...
          tdxApi.getAggregateData(sourceId, JSON.stringify(agePipeline), null, (err, ageResult) => {
            const rObj = {
              areaId: areaId,
              gender: gender,
              serviceRatio: {},
            };
            _.forEach(ageResult.data, (dataObj) => {
      ...
      .catch((err) => {
        console.log(`requesting error with tdxApi ${err.message}`);
      });
  }
