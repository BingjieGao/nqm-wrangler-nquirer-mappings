module.exports = (function() {
  ...
  const ageArray = ["0-4", "5-9", "10-14", "15-19", "20-24", "25-29", "30-34", "35-39", "40-44", "45-49", "50-54", "55-59", "60-64", "65-69", "70-74", "75-79", "80-84", "85-89", "90+"];
    ...
    return tdxApi.getDatasetDataAsync(sourceId, genderFilter, null, genderOptions)
      .then((result) => {
        return Promise.each(result.data, (eachObj) => {
          ...
          const serviceRatio = eachObj.serviceRatio;
          const agePipeline = [
            {
              $match: {
                gender: gender,
          ...
          return new Promise((resolve, reject) => {
            return tdxApi.getAggregateDataAsync(sourceId, JSON.stringify(agePipeline), null)
              .then((result) => {
                ...
                _.forEach(ageArray, (ageBand) => {
                  let total = 0;
                  rObj.ageBand = ageBand;
                  rObj.serviceRatio = {};
                  _.forEach(result.data, (dataObj) => {
                    if (dataObj.ageBand === ageBand) {
                      total += serviceRatio[dataObj.serviceId] * dataObj.ratio;
                      rObj.serviceRatio[dataObj.serviceId] = serviceRatio[dataObj.serviceId] * dataObj.ratio;
                    }
                  });
                  _.forEach(rObj.serviceRatio, (ratio, key) => {
                    rObj.serviceRatio[key] = ratio / total;
                  });
                  //console.log(rObj);
                  dataArray.push(rObj);
                });
